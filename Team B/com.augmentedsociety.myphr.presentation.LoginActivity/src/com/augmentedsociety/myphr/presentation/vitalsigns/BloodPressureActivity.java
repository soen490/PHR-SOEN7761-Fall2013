...
public class BloodPressureActivity extends VitalSignComponent
{
  ...
  private int heart_rate_value=45;
  private int systolic_value=90;
  private int diastolic_value=99;
  Context context ;
...
// to Compare previous Readings
  public String ValueComparer()
	{
		String message =null;
		CommandAction iCA = new CommandAction();
		ArrayList<BloodPressureReading> BloodPressureReadings = iCA.viewBloodPressure(context);
		ArrayList<Float> HeartRate= new ArrayList<Float>();
	  ArrayList<Float> systolic= new ArrayList<Float>();
		ArrayList<Float>  diastolic= new ArrayList<Float>();
		for( BloodPressureReading wr : BloodPressureReadings)
		{
			HeartRate.add(Float.valueOf(wr.getHeartrate()));
			systolic.add(Float.valueOf(wr.getSystolic()));
			diastolic.add(Float.valueOf(wr.getDiastolic()));
				
		}
		if(systolic.size()%2==0 && diastolic.size()%2==0 && HeartRate.size()%2==0 && systolic.size()>=2 && diastolic.size()>=2 && HeartRate.size()>=2 )
		{
			if(((systolic.get(systolic.size()-1)<=90) && (diastolic.get(diastolic.size()-1)<=60)&& (HeartRate.get(HeartRate.size()-1)<60))&&
					((systolic.get(systolic.size()-2)>=140 && systolic.get(systolic.size()-2)<=190) && (diastolic.get(diastolic.size()-2)>=90 && diastolic.get(diastolic.size()-2)<=100)&& (HeartRate.get(HeartRate.size()-2)>100)))
			     {
					message="BloodPressure abruptly decresing";
			     }
				
			if(((systolic.get(systolic.size()-1)<=90) && (diastolic.get(diastolic.size()-1)<=60)&& (HeartRate.get(HeartRate.size()-1)<60))&&
					((systolic.get(systolic.size()-2)<=120) && (diastolic.get(diastolic.size()-2)<=80)&& (HeartRate.get(HeartRate.size()-2)>=60 && HeartRate.get(HeartRate.size()-2)<=100)))
			     {
					message="BloodPressure going down";
			     }
			
			if(((systolic.get(systolic.size()-2)<=90) && (diastolic.get(diastolic.size()-2)<=60)&& (HeartRate.get(HeartRate.size()-2)<60))&&
					((systolic.get(systolic.size()-1)<=120) && (diastolic.get(diastolic.size()-1)<=80)&& (HeartRate.get(HeartRate.size()-1)>=60 && HeartRate.get(HeartRate.size()-1)<=100)))
			     {
					message="BloodPressure is Normal";
			     }
			
			if(((systolic.get(systolic.size()-2)<=90) && (diastolic.get(diastolic.size()-2)<=60)&& (HeartRate.get(HeartRate.size()-2)<60))&&
					((systolic.get(systolic.size()-1)>=140 && systolic.get(systolic.size()-1)<=190) && (diastolic.get(diastolic.size()-1)>=90 && diastolic.get(diastolic.size()-1)<=100)&& (HeartRate.get(HeartRate.size()-1)>100)))
			     {
					message="BloodPressure abruptly increasing";
			     }
			if(((systolic.get(systolic.size()-1)<=90) && (diastolic.get(diastolic.size()-1)<=60)&& (HeartRate.get(HeartRate.size()-1)<60))&&
					((systolic.get(systolic.size()-2)<=90) && (diastolic.get(diastolic.size()-2)<=60)&& (HeartRate.get(HeartRate.size()-2)<60)))
			     {
					message="BloodPressure constantly decresing";
			     }
			
					
			if(((systolic.get(systolic.size()-2)<=120) && (diastolic.get(diastolic.size()-2)<=80)&& (HeartRate.get(HeartRate.size()-2)>=60 && HeartRate.get(HeartRate.size()-2)<=100))&&
					((systolic.get(systolic.size()-1)>=140 && systolic.get(systolic.size()-1)<=190) && (diastolic.get(diastolic.size()-1)>=90 && diastolic.get(diastolic.size()-1)<=100)&& (HeartRate.get(HeartRate.size()-1)>100)))
			     {
					message="BloodPressure is increasing";
			     }
			if(((systolic.get(systolic.size()-1)<=120) && (diastolic.get(diastolic.size()-1)<=80)&& (HeartRate.get(HeartRate.size()-1)>=60 && HeartRate.get(HeartRate.size()-1)<=100))&&
					((systolic.get(systolic.size()-2)>=140 && systolic.get(systolic.size()-2)<=190) && (diastolic.get(diastolic.size()-2)>=90 && diastolic.get(diastolic.size()-2)<=100)&& (HeartRate.get(HeartRate.size()-2)>100)))
			     {
					message="BloodPressure is decreasing";
			     }
