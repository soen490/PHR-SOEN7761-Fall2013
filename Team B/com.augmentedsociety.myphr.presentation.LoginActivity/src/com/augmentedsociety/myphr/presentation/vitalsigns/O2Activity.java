...
//						Toast.LENGTH_LONG).show();
	...
	public String ValueComparer()
	{
		...
		ArrayList<OxygenSaturationReading> TempertureReadings = iCA.viewOxygenSaturation(context);
		ArrayList<Float> Temperature= new ArrayList<Float>();
		for( OxygenSaturationReading wr : TempertureReadings)
		{
			Temperature.add(Float.valueOf(wr.getOxygenSaturation()));
				
		}
		
	
	// temperature Comparison
	if(Temperature.size()%2==0 && Temperature.size()>=2)
		{
		if(MessageCountValue!=Temperature.size())   
		{
			if((Temperature.get(Temperature.size()-1)>=90 && 
		  		Temperature.get(Temperature.size()-2)<90))
		     {
	  	    message="Oxygen level normal detected";
		     }
		  if((Temperature.get(Temperature.size()-2)>=90 && Temperature.get(Temperature.size()-1)<90))
	     {
 	        message="Oxygen level decresing";
	     }
		  MessageCountValue=Temperature.size();
		}
		}
	return message;
	}
