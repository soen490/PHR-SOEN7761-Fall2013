...
public class TemperatureActivity extends VitalSignComponent
{
  ...
  private int Tempterature_threshold=37;// Normal temperature in degree celisius
  Context context;
...
//				Toast.makeText(context, context.getString(R.string.prompt_save_again), Toast.LENGTH_LONG).show();
	...
	public String ValueComparer()
	{
		...
		ArrayList<TemperatureReading> TempertureReadings = iCA.viewTemperature(context);
		ArrayList<Float> Temperature= new ArrayList<Float>();
		ArrayList<Float> Temp_Dif= new ArrayList<Float>();
		for( TemperatureReading wr : TempertureReadings)
		{
			Temperature.add(Float.valueOf(wr.getTemperature()));
				
		}
		// temperature Comparison
		for(int i=0; i< Temperature.size(); i++)
		{ 
			Float value=Temperature.get(i)-Tempterature_threshold;
			Temp_Dif.add(value);
		}
		if(Temp_Dif.size() % 2 == 0 && Temp_Dif.size()>=2 ){
		  if((Temp_Dif.get(Temp_Dif.size()-1)==0.2||Temp_Dif.get(Temp_Dif.size()-1)==0||Temp_Dif.get(Temp_Dif.size()-1)>=-1)&&
		  		(Temp_Dif.get(Temp_Dif.size()-2)<=-2)
			 )
		     {
	  	    message="Temperature is improving";
		     }
		  if((Temp_Dif.get(Temp_Dif.size()-1)>=3)&&
		  		(Temp_Dif.get(Temp_Dif.size()-2)<=-2))
				  {
			  	 message="Temperature abruptly increasing";
				  }
		  if((Temp_Dif.get(Temp_Dif.size()-1)>=3)&&
		  		(Temp_Dif.get(Temp_Dif.size()-2)==0.2||Temp_Dif.get(Temp_Dif.size()-2)==0||Temp_Dif.get(Temp_Dif.size()-2)>=-1)
			 )
		     {
	  	    message="Temperature is increasing";
		     }
		  
		  if((Temp_Dif.get(Temp_Dif.size()-2)==0.2||Temp_Dif.get(Temp_Dif.size()-2)==0||Temp_Dif.get(Temp_Dif.size()-2)>=-1)&&
		  		(Temp_Dif.get(Temp_Dif.size()-1)<=-2)
			 )
		     {
	  	    message="Temperature is decreasing";
		     }
		  if((Temp_Dif.get(Temp_Dif.size()-2)>=3)&&
		  		(Temp_Dif.get(Temp_Dif.size()-1)<=-2))
				  {
			  	 message="Temperature abruptly decreasing";
				  }
		  if((Temp_Dif.get(Temp_Dif.size()-2)>=3)&&
		  		(Temp_Dif.get(Temp_Dif.size()-1)==0.2||Temp_Dif.get(Temp_Dif.size()-1)==0||Temp_Dif.get(Temp_Dif.size()-1)>=-1)
			 )
		     {
	  	    message="Temperature is improving";
		     }
