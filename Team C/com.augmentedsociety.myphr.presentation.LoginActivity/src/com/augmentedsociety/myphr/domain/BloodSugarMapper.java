...
//-------------------------------------------
public static ArrayList<BloodSugarReading> findLastSevenDays(
      Date iLeftBound, Context iContext) throws MapperException
  {
    try
    {
      ArrayList<ArrayList<String>> resultTable = BloodSugarTDG.selectLastSevenDays(String.valueOf(iLeftBound.getTime()), iContext);
      ArrayList<BloodSugarReading> resultReadings = new ArrayList<BloodSugarReading>(resultTable.size());
      for (int i = 0; i < resultTable.size(); ++i)
      {
        ArrayList<String> values = resultTable.get(i);
