...
//-------------------------------------------
public static ArrayList<TemperatureReading> findLastSevenDays(
      Date iLeftBound, Context iContext) throws MapperException
  {
    try
    {
      ArrayList<ArrayList<String>> resultTable = TemperatureTDG.selectLastSevenDays(String.valueOf(iLeftBound.getTime()), iContext);
      ArrayList<TemperatureReading> resultReadings = new ArrayList<TemperatureReading>(resultTable.size());
      for (int i = 0; i < resultTable.size(); ++i)
      {
        ArrayList<String> values = resultTable.get(i);
