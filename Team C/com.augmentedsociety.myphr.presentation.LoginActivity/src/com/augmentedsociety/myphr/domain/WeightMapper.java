...
import com.augmentedsociety.myphr.persistence.WeightTDG;

/**
 * Weight Reading data mapper
 * 
...
public class WeightMapper extends ReadingDataMapper
{
  /**
   * Hidden constructor
   */
  ...
  /**
   * Returns all Weight Reading objects that are in the database.
   * 
   ...
   * @return The ArrayList of all WeightReading in the database
   * @throws MapperException Thrown when there is either a problem with mapping
   *           or if the Persistence layer returns an error
   */
  public static ArrayList<WeightReading> findAll(Context iC) throws MapperException
  {
    try
    {
      ArrayList<ArrayList<String>> resultTable = WeightTDG.selectAll(iC);
  ...
  /**
   * Returns all Weight Reading objects that fall within the time range
   * specified (inclusive)
   * 
   ...
   * @note Recreates only the new objects, since Identity Map is no longer applied: the reason for this is because whenever
   *       data is fetched, we know its usage right away and will not need to keep it in proximity for later usage, but rather direct usage
   * @return The ArrayList of all weighteReading in the database that
   *         fall within range
   * @throws MapperException Thrown when there is either a problem with mapping
   *           or if the Persistence layer returns an error
   */
      ...
      Date iLeftBound, Date iRightBound, Context iC) throws MapperException
  {
    try
    {
      ArrayList<ArrayList<String>> resultTable = WeightTDG
  ...
  /**
   * Stores the new WeightReading
   * 
   * @param r The WeightReading object
   * @param c The application context
   * @throws MapperException Thrown if there is a problem mapping or if the
   *           persistence returns an error
   */
  public static void insert(WeightReading iR, Context iC) throws MapperException
  {
    try
    {
      ...
      values.add(String.valueOf(iR.getWeight()));

      WeightTDG.insert(values, iC);
      WeightTDG.removeRedundantEntries(values, iC);
    } 
    catch (PersistenceException iE)
    {
      throw new MapperException(
  ...
  /**
   * Finds the next ID available for using in Weight Reading
   * initialization
   * 
   * @param c The application context
  ...
  public static Long getNextAvailableId(Context iC) throws MapperException
  {
    try
    {
      Long id = WeightTDG.getNextAvailableId(iC);
      return id;
    } 
    catch (PersistenceException iE)
    {
...
//-------------------------------------------
public static ArrayList<WeightReading> findLastSevenDays(
      Date iLeftBound, Context iContext) throws MapperException
  {
    try
    {
      ArrayList<ArrayList<String>> resultTable = WeightTDG.selectLastSevenDays(String.valueOf(iLeftBound.getTime()), iContext);
      ArrayList<WeightReading> resultReadings = new ArrayList<WeightReading>(resultTable.size());
      for (int i = 0; i < resultTable.size(); ++i)
      {
        ArrayList<String> values = resultTable.get(i);
