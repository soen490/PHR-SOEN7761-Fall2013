...
/**
 * Table Data Gateway for Blood Sugar readings
 * 
 * @author Serge-Antoine Naim
 * 
 */

public class BloodSugarTDG extends MeasurementTDG
{
	public static final String TABLE = "blood_sugar";

	private static final String SELECT_ALL = "SELECT r.id, r.when_taken, r.source, r.blood_glucose FROM "
			+ TABLE + " as r;";

	private static final String SELECT_BETWEEN_DATES_INCLUSIVE = "SELECT r.id, r.when_taken, r.source, r.blood_glucose FROM "
			+ TABLE + " as r WHERE r.when_taken >= ? AND r.when_taken <= ?;";
	
  private static final String SELECT_LAST_SEVEN_DAYS = 
  		"SELECT r.id, r.when_taken, r.source, r.blood_glucose FROM "
  		+ TABLE
	...
	/**
	 * Selects all Blood Sugar readings from the database
	 * 
	 * @param c
	 ...
	 *         respective order: id, when_taken, source, blood_glucose
	...
	/**
	 * This method insets one single Blood Sugar reading into the DB.
	 * 
	 * @param values
			...
			throws PersistenceException
	{
		try
		{
			...
			wValues.put("blood_glucose", iValues.get(3));
      ...
      throws PersistenceException
  {
    try
    {
      ...
      Cursor cursor = wDatabase.rawQuery(SELECT_LAST_SEVEN_DAYS, args);
