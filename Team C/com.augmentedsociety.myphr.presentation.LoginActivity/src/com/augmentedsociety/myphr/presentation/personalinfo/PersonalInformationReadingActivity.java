...
public class PersonalInformationReadingActivity extends PersonalInfoComponent
{
	...
	public View fillExistingView(View iView)
	{
		...
		try
		{
			...
			if( currentProfile.getPhoneNumber() > (long)0 )
			{
...
//			d.init(cal.get(Calendar.YEAR), cal.get(Calendar.MONTH), cal.get(Calendar.DAY_OF_MONTH), null);
  ...
  public void submit(View iView)
  {
	    ...
	    TextView first, last, phone, address, email, medicare, insurance, contact1, contact2;
			...
			String tfirst, tlast, taddress, temail, tmedicare, tinsurance, tcontact1, tcontact2 = "";
			long tphone = (long)0;
			...
			tcontact2 = contact2.getText().toString();
				...
				if(exists)
				{
					ProfileMapper.update(new ProfileReading(mID,tlast, tfirst, tDate, tphone, taddress,
							temail, tmedicare, tinsurance, tcontact1, tcontact2), context);
				}
				else
				{
					
					ProfileReading p = new ProfileReading( ProfileMapper.getNextAvailableId(context),tlast, tfirst, tDate, tphone, taddress,
							temail, tmedicare, tinsurance, tcontact1, tcontact2);
