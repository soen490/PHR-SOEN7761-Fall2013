    ...
    Context iContext = iView.getContext();
		...
		if (this != null){
				...
				// Get today as a Calendar  
					...
					ArrayList<TemperatureReading> result = TemperatureMapper.findLastSevenDays(lastSevenDays, iContext);
					TextView t = (TextView) mAct.findViewById(R.id.temperature_value);
					
					float tTemperature = Float.valueOf(t.getText().toString());

				  if (tTemperature > 37.5 || tTemperature<36.5){
				  	isDataValid=false;
				  	return;
				  }
				  int averageTemperature = 0;
				  for (TemperatureReading bp: result){	
				  	averageTemperature += bp.getTemperature();
				  }
				  if (result.size() ==0){
				  	isDataValid=true;
				  	return;
				  }
				  averageTemperature=averageTemperature/result.size();

				  float diff=0;
				  boolean diffTemperature = false;
				   
				  if (averageTemperature == tTemperature){
				  	diff=0;
				  } else if (averageTemperature > tTemperature){
				  	 diff = averageTemperature - tTemperature;
				  } else if (averageTemperature < tTemperature){
				  	 diff = tTemperature-averageTemperature;
				  }
				  
				  if (diff >= 0.5){
				  	diffTemperature = true;
				  }
