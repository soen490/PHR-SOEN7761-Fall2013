...
public class VitalSignsActivity extends MenuActivity
{
	...
	private String lastActivity ="";//fh
	
	protected static boolean hasBeenSaved = false;
	...
	/**
	 * Initialize the times where the user should enter data
	 * based on the previous entries in the last seven days
	 */
	//el
	private void initializeTimeReminders(){
		try
			{
			int entries=0;
...
//			get last 7 days
			ArrayList<BloodPressureReading> bpResult = BloodPressureMapper
					.findLastSevenDays(lastSevenDays, getApplicationContext());
			ArrayList<Integer> bpHours = new ArrayList<Integer>();
			ArrayList<BloodSugarReading> bsResult = BloodSugarMapper
					.findLastSevenDays(lastSevenDays, getApplicationContext());
			ArrayList<TemperatureReading> temperatureResult = TemperatureMapper
					.findLastSevenDays(lastSevenDays, getApplicationContext());
			ArrayList<WeightReading> weightResult = WeightMapper
					.findLastSevenDays(lastSevenDays, getApplicationContext());
			ArrayList<OxygenSaturationReading> o2Result = OxygenSaturationMapper
					.findLastSevenDays(lastSevenDays, getApplicationContext());
			Calendar c = GregorianCalendar.getInstance();
			HashSet<Integer> uniqueDays = new HashSet<Integer>();
			// check number of days entered
			for (BloodPressureReading bpr : bpResult)
			{
				c.setTime(bpr.getDateTaken());
				uniqueDays.add(c.get(Calendar.DAY_OF_YEAR));
				// for future use....
				bpHours.add(c.get(Calendar.HOUR));
				entries++;
			}
			for (OxygenSaturationReading o2r : o2Result)
			{
				c.setTime(o2r.getDateTaken());
				uniqueDays.add(c.get(Calendar.DAY_OF_YEAR));
				// for future use....
				bpHours.add(c.get(Calendar.HOUR));
				entries++;
			}
			for (BloodSugarReading bsr : bsResult)
			{
				c.setTime(bsr.getDateTaken());
				uniqueDays.add(c.get(Calendar.DAY_OF_YEAR));
				// for future use....
				bpHours.add(c.get(Calendar.HOUR));
				entries++;
			}
			for (TemperatureReading temperature : temperatureResult)
			{
				c.setTime(temperature.getDateTaken());
				uniqueDays.add(c.get(Calendar.DAY_OF_YEAR));
				// for future use....
				bpHours.add(c.get(Calendar.HOUR));
				entries++;
			}
			for (WeightReading weight : weightResult)
			{
				c.setTime(weight.getDateTaken());
				uniqueDays.add(c.get(Calendar.DAY_OF_YEAR));
				// for future use....
				bpHours.add(c.get(Calendar.HOUR));
				entries++;
			...
			int nbReminders = entries / uniqueDays.size();
			double nbTimeBlocksWithDecimals = 24 / nbReminders;
			int timeBlockHours = (int) Math.round(nbTimeBlocksWithDecimals);
			Collections.sort(bpHours);
			int numberOfBlocks = (int) 24 / timeBlockHours;
			ArrayList<ArrayList<Integer>> timeBlocks = new ArrayList<ArrayList<Integer>>();
			// initialize array
			for (int i = 0; i < numberOfBlocks; i++)
			{
				timeBlocks.add(new ArrayList<Integer>());
			}
			// add hours in timeblocks
			for (int i = 0; i < bpHours.size(); i++)
			{
				int index = bpHours.get(i) / timeBlockHours;
				timeBlocks.get(index).add(bpHours.get(i));
			}
			// average time per time block to create reminders
			ArrayList<Double> averageTime = new ArrayList<Double>();
			int avg = 0;
			for (int i = 0; i < timeBlocks.size(); i++)
			{
				avg = 0;
				for (int j = 0; j < timeBlocks.get(i).size(); j++)
				{
					avg += timeBlocks.get(i).get(j);
				}
				averageTime.add((double) avg / timeBlocks.get(i).size());
			}
			createReminders(averageTime);
	...
	/**
	 * This function will send out a popup to remind user to enter data
	 * Actually creates the new notification
	 * based on method in NewNotificationActivity Class
	 * 
	 * @param view
	 */
	public void createReminders(ArrayList<Double> averageTime)
	{//el
		String title = "You seem to have forgotten to enter your data in myPHR app! This is a friendly reminder!";

		for (int i=0; i< averageTime.size(); i++){
			int hour = (int) Math.floor(averageTime.get(i));
			int minute = (int)(Math.abs(averageTime.get(i))-hour)*60;
		...
		try
		{
			...
			Notification r = new Notification(id, c.getTime(), isRecurring,
					0, true, title);
	...
	{
		final View iView = (View) findViewById(R.id.menuButton);
		boolean bloodPressure;
		if (mLastBloodPressureAct != null)
		{
			mLastBloodPressureAct.validateData(iView);
			bloodPressure = mLastBloodPressureAct.isDataValid;
			lastActivity = "bp";
		}else{
			bloodPressure= true;
		}
		...
		{
			mLastWeightAct.validateData(iView);
			weight = mLastWeightAct.isDataValid;
			lastActivity = "weight";
		}else{
			weight= true;
		}
		...
		if (mLastO2Act != null)
		{
			mLastO2Act.validateData(iView);
			oxygen =mLastO2Act.isDataValid;
			lastActivity = "oxygen";
		}else{
			oxygen= true;
		}
		...
		{
			mLastTemperatureAct.validateData(iView);
			temperature = mLastTemperatureAct.isDataValid;
			lastActivity = "temperature";
		}else{
			temperature= true;
		}
		...
		{	
			mLastBloodSugarAct.validateData(iView);
			bloodSugar = mLastBloodSugarAct.isDataValid;
			lastActivity = "bloodSugar";
		}else{
			bloodSugar= true;
		}
		isDataValid = bloodPressure && weight && oxygen && temperature && bloodSugar;
		
		if ( !bloodPressure || !weight || !oxygen || !temperature || !bloodSugar )
		{
			Context iContext = iView.getContext();
			String invalidEntries ="";
			if(!bloodPressure){
				invalidEntries+="Blood Pressure, ";
			}
			if(!weight){
				invalidEntries+="Weight, ";
			}
			if(!oxygen){
				invalidEntries+="Oxygen, ";
			}
			if(!temperature){
				invalidEntries+="Temperature, ";
			}
			if(!bloodSugar){
				invalidEntries+="Blood Sugar, ";
			}
			invalidEntries =invalidEntries.substring(0,invalidEntries.lastIndexOf(","));

			
      final View tempView = iView;
      new AlertDialog.Builder(iContext).setTitle("Have you made a mistake?")
      .setMessage("The following seems to be wrong: "+ invalidEntries+". Are the data correct or would you like to change them?")
      .setNegativeButton("No, the data is not correct.", new DialogInterface.OnClickListener(){
      	public void onClick(DialogInterface dialog, int which) {
        	isDataValid=false;
      	}
      })
      .setPositiveButton("Yes, please save!", new DialogInterface.OnClickListener(){
      	public void onClick(DialogInterface dialog, int which) {
        	isDataValid=true;
//        	((VitalSignsActivity) VitalSignsActivity.VITAL_SIGN_ACTIVITY)
//					.saveCurrentData();
  //      	TextView v = (TextView) mAct.findViewById(R.id.blood_sugar_value);
        	//saveMethod(v);
        	saveData(iView);
      	}
      })
	...
	private void saveData(View iView)
	{
		
		if (mLastBloodPressureAct != null)
			if(lastActivity.equals("bp"))
			{
				mLastBloodPressureAct.submit(iView, mDate, true);
			}
			else
			{
				mLastBloodPressureAct.submit(iView, mDate, false);
			}
		
		if (mLastWeightAct != null)
			if(lastActivity.equals("weight"))
			{
				mLastWeightAct.submit(iView, mDate, true);
			}
			else
			{
				mLastWeightAct.submit(iView, mDate,false);
			}

		if (mLastO2Act != null)
			if(lastActivity.equals("oxygen"))
			{
				mLastO2Act.submit(iView, mDate, true);
			}
			else
			{
				mLastO2Act.submit(iView, mDate,false);
			}

		if (mLastTemperatureAct != null)
			if(lastActivity.equals("temperature"))
			{
				mLastTemperatureAct.submit(iView, mDate, true);
			}
			else
			{
				mLastTemperatureAct.submit(iView, mDate,false);
			}

		if (mLastBloodSugarAct != null)
			if(lastActivity.equals("bloodSugar"))
			{
				mLastBloodSugarAct.submit(iView, mDate, true);
			}
			else
			{
				mLastBloodSugarAct.submit(iView, mDate, false);
			}
