...
public class ProfileMapper extends ReadingDataMapper
{
			...
			throws MapperException
	{
		try
		{
			...
			for (int i = 0; i < resultTable.size(); ++i)
			{
				...
				String note = values.get(11);
				String room = values.get(12);
				String bed = values.get(13);
				r = new ProfileReading(id,lastName, firstName, DOB, phoneNumber, address,
						email, medicare, insurance, emergencyContact1, emergencyContact2,note,room,bed);
   ...
   * @param mID The id of the ProfileReading to get from the DB
   * @param iContext Application context
   * @return
   * @throws MapperException
   */
  public static ProfileReading getOne(long mID, Context iContext) throws MapperException
  {
    try
    {
      ArrayList<String> values = ProfileTDG.select(mID, iContext);
  ...
  public static ProfileReading getOne_BY_ROOM(String iroom,String ibed, Context iContext) throws MapperException
  {
    try
    {
      ArrayList<String> values = ProfileTDG.select_By_Room(iroom,ibed, iContext);
  ...
  public static ProfileReading getOne_BY_Medicare(String imidcare, Context iContext) throws MapperException
  {
    try
    {
      ArrayList<String> values = ProfileTDG.select_By_Medicare(imidcare, iContext);
			...
			throws MapperException
	{
		try
		{
			ArrayList<String> values = new ArrayList<String>(14);
			...
			values.add(iR.getNote().toString());
			values.add(iR.getRoom().toString());
			values.add(iR.getBed().toString());
